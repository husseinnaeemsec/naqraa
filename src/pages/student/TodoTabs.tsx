...
export default function TodoTabs({ onChange, tasks, collections }: Props) {
    ...
    const tabs = [
        {
            title: t('todo_tabs.today'),
            id: 'today',
            icon: 'fi fi-rr-cloud-sun'
        },
        {
            title: t('todo_tabs.repeat'),
            id: "repeat",
            ...
            <ContextMenu>
                <ContextMenuTrigger>
                    <div className="w-full p-4 dark:bg-emerald-950 bg-white sticky top-0  z-0   border-x h-dynamic">
                        <small className="text-slate-500 block mb-2">{t('todo_tabs.tasks_label')}</small>
                        {
                            tabs.map((tab) => {
                        ...
                        <small className="text-slate-500 block mb-2">{t('todo_tabs.collections_label')}</small>
                ...
                <ContextMenuContent >
                    ...
                    <ContextMenuItem dir="rtl">
                        ...
                        {t("todo_tabs.add_new_task")}
                    </ContextMenuItem>
                    <ContextMenuItem dir="rtl">
                        <i className="fi fi-rr-folder-open text-sm mr-2" />
                        {t("todo_tabs.add_new_collection")}
                    </ContextMenuItem>
                </ContextMenuContent>
            </ContextMenu>
        </>
